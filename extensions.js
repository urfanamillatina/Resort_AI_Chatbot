  ...
  render: ({ trace, element }) => {
    ...
    formContainer.innerHTML = `
          <style>
            ...
            .submit {
              background: linear-gradient(to right, #339134, #339134 );
              border: none;
              color: white;
              padding: 10px;
              border-radius: 5px;
          ...
          <label for="name">Full name as per ID/ Passport</label>
...
export const DateExtension = {
  ...
  render: ({ trace, element }) => {
    ...
    // Convert to ISO string and get the date part (YYYY-MM-DD)
    let minDateString = minDate.toISOString().split('T')[0]; // Get only the date part
    let maxDateString = maxDate.toISOString().split('T')[0]; // Get only the date part

    // Function to format the date as DD-name of the month-YYYY
    ...
    formContainer.innerHTML = `
      ...
      <div class="meeting">
        <input type="date" id="checkin" name="checkin" min="${minDateString}" max="${maxDateString}" />
      </div>
      <label for="checkout">Check-out Date</label>
      <div class="meeting">
        <input type="date" id="checkout" name="checkout" min="${minDateString}" max="${maxDateString}" />
      </div>
    ...
    checkinInput.addEventListener('input', function () {
      checkoutInput.min = checkinInput.value; // Ensure checkout date is after checkin date
      validateDates();
    });

    checkoutInput.addEventListener('input', function () {
      checkinInput.max = checkoutInput.value; // Ensure checkin date is before checkout date
      validateDates();
    });
    ...
    formContainer.addEventListener('submit', function (event) {
      ...
      const checkinDate = formatDate(checkinInput.value);
      const checkoutDate = formatDate(checkoutInput.value);
